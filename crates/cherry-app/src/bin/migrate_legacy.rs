...
use cherry_services::{AppServicesBuilder, import_legacy_json};

fn main() -> anyhow::Result<()> {
    let mut args = env::args().skip(1);
    let Some(legacy_json) = args.next() else {
        eprintln!(
            "Usage: cargo run -p cherry-app --bin migrate_legacy -- <legacy-export.json> [db-path]"
        );
        std::process::exit(2);
    };
    ...
    let report = import_legacy_json(&services, legacy_json)?;
