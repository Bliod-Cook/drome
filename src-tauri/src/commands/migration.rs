...
fn cherry_config_path() -> Option<PathBuf> {
  ...
  Some(home.join(HOME_CHERRY_DIR).join("config").join("config.json"))
}
...
fn read_cherry_data_paths(config_path: &Path) -> Vec<PathBuf> {
  ...
  match app_data_path {
    ...
    Some(serde_json::Value::Array(arr)) => arr
      .iter()
      .filter_map(|item| item.get("dataPath").and_then(|v| v.as_str()).map(PathBuf::from))
      .collect(),
    _ => vec![],
  }
}
...
pub fn migration_detect(state: &State<'_, AppState>) -> Result<MigrationDetectResult> {
  ...
  if let Some(home) = dirs::home_dir() {
    ...
    #[cfg(target_os = "macos")]
    {
      dirs.push(home.join("Library").join("Application Support").join("CherryStudio"));
    }
...
pub fn migration_copy_data_dir(app: &AppHandle, window: &WebviewWindow, state: &State<'_, AppState>, old_user_data_dir: String) -> Result<()> {
  ...
  let entries: Vec<_> = WalkDir::new(&old_dir).into_iter().filter_map(|e| e.ok()).collect();
