...
fn span_id(entity: &Value) -> Option<String> {
  entity.get("id").and_then(|v| v.as_str()).map(|s| s.to_string())
}

fn trace_id(entity: &Value) -> Option<String> {
  entity.get("traceId").and_then(|v| v.as_str()).map(|s| s.to_string())
}

fn model_name(entity: &Value) -> Option<String> {
  entity.get("modelName").and_then(|v| v.as_str()).map(|s| s.to_string())
}
...
fn unbound_trace_path(state: &State<'_, AppState>, trace_id: &str) -> PathBuf {
  trace_dir(state).join("_unbound").join(format!("{trace_id}.json"))
}

fn trace_file_path(state: &State<'_, AppState>, topic_id: &str, trace_id: &str) -> PathBuf {
  trace_dir(state).join(topic_id).join(format!("{trace_id}.json"))
}
...
fn upsert_span(spans: &mut Vec<Value>, entity: &Value) {
  ...
  if let Some(existing) = spans.iter_mut().find(|v| v.get("id").and_then(|vv| vv.as_str()) == Some(id.as_str())) {
...
pub fn trace_bind_topic(state: &State<'_, AppState>, topic_id: String, trace_id: String) -> Result<()> {
...
pub fn trace_get_data(state: &State<'_, AppState>, topic_id: String, trace_id: String, model_name: Option<String>) -> Result<Vec<Value>> {
...
pub fn trace_clean_history(state: &State<'_, AppState>, topic_id: String, trace_id: String, model_name: Option<String>) -> Result<()> {
  let path = trace_file_path(state, &topic_id, &trace_id);
  if !path.exists() {
    return Ok(());
  }
...
pub fn trace_clean_topic(state: &State<'_, AppState>, topic_id: String, trace_id: Option<String>) -> Result<()> {
...
pub fn trace_add_end_message(_state: &State<'_, AppState>, _span_id: String, _model_name: String, _message: String) -> Result<()> {
  // Best-effort no-op for now.
  Ok(())
}

pub fn trace_add_stream_message(_state: &State<'_, AppState>, _span_id: String, _model_name: String, _context: String, _message: Value) -> Result<()> {
  // Best-effort no-op for now.
  Ok(())
}
...
fn store_language(state: &State<'_, AppState>) -> Option<String> {
  ...
  value.get("language").and_then(|v| v.as_str()).map(|s| s.to_string())
}
...
fn get_or_create_trace_window(app: &AppHandle) -> Result<WebviewWindow> {
  ...
  WebviewWindowBuilder::new(app, TRACE_WINDOW_LABEL, WebviewUrl::App("traceWindow.html".into()))
    .title("Trace")
...
pub fn trace_set_title(window: &WebviewWindow, title: String) -> Result<()> {
  window.set_title(&title).map_err(|e| DromeError::Message(e.to_string()))?;
  Ok(())
}
