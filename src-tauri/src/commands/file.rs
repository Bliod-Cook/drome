...
fn file_type_by_ext_or_content(path: &Path) -> FileTypes {
  ...
  const TEXT_EXTS: &[&str] = &[
    ".txt", ".md", ".markdown", ".json", ".csv", ".yaml", ".yml", ".log", ".html", ".htm", ".xml", ".js", ".ts", ".tsx", ".jsx",
    ".css", ".ini", ".toml", ".rs", ".py", ".java", ".go", ".c", ".cpp", ".h", ".hpp",
  ];
  const DOC_EXTS: &[&str] = &[".pdf", ".doc", ".docx", ".ppt", ".pptx", ".xls", ".xlsx", ".epub", ".rtf"];
...
pub fn file_open(app: &AppHandle, state: &State<'_, AppState>, options: Option<Value>) -> Result<Option<FileOpenResult>> {
  let filters = options
    .as_ref()
...
pub fn file_select_folder(app: &AppHandle, state: &State<'_, AppState>, options: Option<Value>) -> Result<Option<String>> {
  let title = options
    .as_ref()
    .and_then(|v| v.get("title"))
    .and_then(|v| v.as_str())
...
pub fn file_save(app: &AppHandle, state: &State<'_, AppState>, args: Vec<Value>) -> Result<Option<String>> {
  let suggested_name = args.get(0).and_then(|v| v.as_str()).unwrap_or("output.txt").to_string();
  ...
  let mut dialog = tauri_plugin_dialog::DialogExt::dialog(app).file().set_file_name(&suggested_name);
  if let Some(t) = title {
    dialog = dialog.set_title(t);
  }
  if !filters.is_empty() {
...
pub fn file_select(app: &AppHandle, state: &State<'_, AppState>, options: Option<Value>) -> Result<Option<Vec<StoredFileMetadata>>> {
  let filters = options
    .as_ref()
    .and_then(|v| v.get("filters"))
    .and_then(|v| v.as_array())
  ...
  let selected = if multi { dialog.blocking_pick_files() } else { dialog.blocking_pick_file().map(|p| vec![p]) };
  ...
  for fp in paths {
    let path = fp.into_path().map_err(|e| DromeError::Message(e.to_string()))?;
    let parent = path.parent().unwrap_or(&path).to_path_buf();
    allow_dir(state, &parent);
    ...
    let ext = {
      ...
      if e.is_empty() { "".to_string() } else { format!(".{}", e.to_string()) }
    };
    let file_type = file_type_by_ext_or_content(&path);
    out.push(StoredFileMetadata {
      id: Uuid::new_v4().to_string(),
      origin_name: path.file_name().unwrap_or_default().to_string_lossy().to_string(),
...
pub fn file_get(state: &State<'_, AppState>, file_path: String) -> Result<Option<StoredFileMetadata>> {
...
fn find_duplicate_file(state: &State<'_, AppState>, source_path: &Path) -> Result<Option<StoredFileMetadata>> {
  ...
  for entry in fs::read_dir(dir)? {
    ...
    let Ok(stored_meta) = fs::metadata(&p) else { continue };
    ...
    if let Ok(stored_hash) = md5_hex(&p) {
      if stored_hash == src_hash {
        ...
        return Ok(Some(StoredFileMetadata {
          id,
          origin_name: p.file_name().unwrap_or_default().to_string_lossy().to_string(),
...
pub fn file_upload(state: &State<'_, AppState>, file: StoredFileMetadata) -> Result<StoredFileMetadata> {
  ...
  let origin_name = source.file_name().unwrap_or_default().to_string_lossy().to_string();
...
pub fn file_move_dir(state: &State<'_, AppState>, dir_path: String, new_dir_path: String) -> Result<()> {
...
pub fn file_rename(state: &State<'_, AppState>, file_path: String, new_name: String) -> Result<()> {
  ...
  let parent = src.parent().ok_or_else(|| DromeError::Message("Invalid path".into()))?;
...
pub fn file_rename_dir(state: &State<'_, AppState>, dir_path: String, new_name: String) -> Result<()> {
  let src = normalize_path(&dir_path);
  if !is_allowed(state, &src) {
    return Err(DromeError::Message("Path not allowed".into()));
  }
...
fn extract_docx_text(path: &Path) -> Result<String> {
  ...
  while i < xml.len() {
    if xml[i..].starts_with("<w:tab") {
      out.push('\t');
    } else if xml[i..].starts_with("<w:br") || xml[i..].starts_with("<w:cr") || xml[i..].starts_with("</w:p") {
      out.push('\n');
...
pub fn file_read(state: &State<'_, AppState>, file_id: String, detect_encoding: bool) -> Result<String> {
...
pub fn file_read_external(state: &State<'_, AppState>, file_path: String, detect_encoding: bool) -> Result<String> {
  let path = normalize_path(&file_path);
  if !is_allowed(state, &path) {
    return Err(DromeError::Message("Path not allowed".into()));
  }
...
pub fn file_list_directory(state: &State<'_, AppState>, dir_path: String, options: Option<Value>) -> Result<Vec<String>> {
  let path = normalize_path(&dir_path);
  if !is_allowed(state, &path) {
    return Err(DromeError::Message("Path not allowed".into()));
  }
...
pub fn file_check_file_name(state: &State<'_, AppState>, dir_path: String, file_name: String, is_file: bool) -> Result<FileNameGuardResult> {
...
pub fn file_validate_notes_directory(state: &State<'_, AppState>, dir_path: String) -> Result<bool> {
  let path = normalize_path(&dir_path);
...
fn scan_notes_dir(base: &Path, current: &Path, depth: usize) -> Result<Vec<NotesTreeNode>> {
  ...
  for entry in entries {
    ...
    if meta.is_file() {
      ...
      let external = path_to_string(&path);
      let id = format!("{:x}", md5::compute(external.as_bytes()));
      let file_stem = path.file_stem().and_then(|s| s.to_str()).unwrap_or(&name).to_string();

      let dir_rel = rel.parent().map(|p| p.to_string_lossy().replace('\\', "/")).unwrap_or_default();
      let tree_path = if dir_rel.is_empty() || dir_rel == "." {
        format!("/{file_stem}")
      } else {
        format!("/{}/{}", dir_rel.trim_start_matches("./").trim_start_matches('/'), file_stem)
      };
...
pub fn file_get_directory_structure(state: &State<'_, AppState>, dir_path: String) -> Result<Vec<NotesTreeNode>> {
  let base = normalize_path(&dir_path);
  if !is_allowed(state, &base) {
    return Err(DromeError::Message("Path not allowed".into()));
  }
...
pub fn file_open_with_relative_path(state: &State<'_, AppState>, file: StoredFileMetadata) -> Result<()> {
...
pub fn file_save_image(app: &AppHandle, state: &State<'_, AppState>, name: String, data: String) -> Result<()> {
...
pub fn file_save_base64_image(state: &State<'_, AppState>, base64_data: String) -> Result<StoredFileMetadata> {
  ...
  Ok(StoredFileMetadata {
    id: uuid,
    origin_name: dest.file_name().unwrap_or_default().to_string_lossy().to_string(),
    name: dest.file_name().unwrap_or_default().to_string_lossy().to_string(),
    path: path_to_string(&dest),
...
pub fn file_save_pasted_image(state: &State<'_, AppState>, image_data: Vec<u8>, extension: Option<String>) -> Result<StoredFileMetadata> {
...
pub fn file_base64_file(state: &State<'_, AppState>, id: String) -> Result<Base64FileResult> {
  ...
  let mime = MimeGuess::from_path(&path).first_or_octet_stream().to_string();
  Ok(Base64FileResult { data: base64, mime })
}

pub fn file_pdf_page_count(state: &State<'_, AppState>, id: String) -> Result<u32> {
  let path = files_dir(state).join(id);
  let doc = lopdf::Document::load(path).map_err(|e| DromeError::Message(format!("Failed to load pdf: {e}")))?;
...
pub fn file_base64_image(state: &State<'_, AppState>, id: String) -> Result<Base64ImageResult> {
  ...
  let ext = ext_lower(&path);
  let ext = if ext == ".jpg" { "jpeg".into() } else { ext.trim_start_matches('.').to_string() };
...
pub fn file_download(state: &State<'_, AppState>, url: String, is_use_content_type: Option<bool>) -> Result<StoredFileMetadata> {
  let client = reqwest::blocking::Client::builder()
    .user_agent("Mozilla/5.0 (compatible; drome/0.1)")
    .build()
    .map_err(|e| DromeError::Message(e.to_string()))?;

  let resp = client.get(&url).send().map_err(|e| DromeError::Message(e.to_string()))?;
  if !resp.status().is_success() {
    return Err(DromeError::Message(format!("Download failed: HTTP {}", resp.status())));
  }

  let headers = resp.headers().clone();
  let bytes = resp.bytes().map_err(|e| DromeError::Message(e.to_string()))?.to_vec();
  ...
  let mut filename = headers
    ...
    .map(|s| s.to_string())
    .unwrap_or_else(|| {
      url.split('/').last().unwrap_or("download").split('?').next().unwrap_or("download").to_string()
    });
...
pub fn file_batch_upload_markdown(state: &State<'_, AppState>, file_paths: Vec<String>, target_path: String) -> Result<BatchUploadMarkdownResult> {
