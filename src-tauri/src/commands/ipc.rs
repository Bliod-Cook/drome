...
fn arg<T: serde::de::DeserializeOwned>(args: &[Value], idx: usize) -> Result<T> {
    ...
    serde_json::from_value(value)
        .map_err(|e| DromeError::Message(format!("Invalid arg at index {idx}: {e}")))
}
...
fn opt_arg<T: serde::de::DeserializeOwned>(args: &[Value], idx: usize) -> Result<Option<T>> {
    ...
    Ok(Some(serde_json::from_value(value.clone()).map_err(
        |e| DromeError::Message(format!("Invalid arg at index {idx}: {e}")),
    )?))
}
...
pub async fn ipc_invoke(
    ...
    (|| -> Result<Value> {
        match channel.as_str() {
            ...
            "open:website" => {
                to_value(commands::app::open_website(&app, arg::<String>(&args, 0)?)?)
            }
            ...
            "app:get-disk-info" => {
                to_value(commands::app::app_get_disk_info(arg::<String>(&args, 0)?)?)
            }
            "app:get-data-path-from-args" => {
                to_value(commands::app::app_get_data_path_from_args()?)
            }
            "app:select" => to_value(commands::app::app_select(
                &app,
                &state,
                args.get(0).cloned(),
            )?),
            "app:is-not-empty-dir" => to_value(commands::app::app_is_not_empty_dir(
                arg::<String>(&args, 0)?,
            )?),
            ...
            "app:set-app-data-path" => to_value(commands::app::app_set_app_data_path(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            "app:relaunch-app" => {
                to_value(commands::app::app_relaunch_app(&app, args.get(0).cloned())?)
            }
            "app:set-full-screen" => to_value(commands::app::app_set_full_screen(
                &window,
                arg::<bool>(&args, 0)?,
            )?),
            ...
            "app:set-theme" => to_value(commands::app::app_set_theme(
                &app,
                &state,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "window:reset-minimum-size" => {
                to_value(commands::window::window_reset_minimum_size(&window)?)
            }
            "window:get-size" => to_value(commands::window::window_get_size(&window)?),

            // Config
            "config:get" => to_value(commands::config::config_get(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            ...
            // File dialogs and fs
            "file:open" => to_value(commands::file::file_open(
                &app,
                &state,
                args.get(0).cloned(),
            )?),
            "file:openPath" => to_value(commands::file::file_open_path(
                &app,
                arg::<String>(&args, 0)?,
            )?),
            "file:select" => to_value(commands::file::file_select(
                &app,
                &state,
                args.get(0).cloned(),
            )?),
            "file:selectFolder" => to_value(commands::file::file_select_folder(
                &app,
                &state,
                args.get(0).cloned(),
            )?),
            ...
            "file:mkdir" => to_value(commands::file::file_mkdir(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            "file:upload" => to_value(commands::file::file_upload(
                &state,
                arg::<commands::file::StoredFileMetadata>(&args, 0)?,
            )?),
            "file:get" => to_value(commands::file::file_get(&state, arg::<String>(&args, 0)?)?),
            "file:createTempFile" => to_value(commands::file::file_create_temp_file(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            "file:delete" => to_value(commands::file::file_delete(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            "file:deleteDir" => to_value(commands::file::file_delete_dir(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            "file:deleteExternalFile" => to_value(commands::file::file_delete_external_file(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            "file:deleteExternalDir" => to_value(commands::file::file_delete_external_dir(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "file:base64Image" => to_value(commands::file::file_base64_image(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            "file:saveBase64Image" => to_value(commands::file::file_save_base64_image(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "file:base64File" => to_value(commands::file::file_base64_file(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            "file:getPdfInfo" => to_value(commands::file::file_pdf_page_count(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            "file:binaryImage" => to_value(commands::file::file_binary_image(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "file:isTextFile" => to_value(commands::file::file_is_text_file(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            "file:isDirectory" => to_value(commands::file::file_is_directory(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "file:getDirectoryStructure" => to_value(commands::file::file_get_directory_structure(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "file:validateNotesDirectory" => to_value(
                commands::file::file_validate_notes_directory(&state, arg::<String>(&args, 0)?)?,
            ),
            ...
            "file:showInFolder" => to_value(commands::file::file_show_in_folder(
                &app,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "fs:readText" => to_value(commands::fs::fs_read_text(
                &app,
                &state,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "app:has-write-permission" => {
                to_value(commands::file::has_write_permission(arg::<String>(
                    &args, 0,
                )?)?)
            }
            ...
            // Backup (zip)
            "backup:backup" => to_value(commands::backup::backup_backup(
                &app, &window, &state, args,
            )?),
            ...
            "backup:restoreFromLocalBackup" => {
                to_value(commands::backup::restore_from_local_backup(
                    &app,
                    &window,
                    &state,
                    arg::<String>(&args, 0)?,
            ...
            "backup:listLocalBackupFiles" => to_value(commands::backup::list_local_backup_files(
                opt_arg::<String>(&args, 0)?,
            )?),
            "backup:deleteLocalBackupFile" => to_value(commands::backup::delete_local_backup_file(
                arg::<String>(&args, 0)?,
                opt_arg::<String>(&args, 1)?,
            )?),
            "backup:createLanTransferBackup" => {
                to_value(commands::backup::create_lan_transfer_backup(
                    &app,
                    &window,
                    &state,
                    arg::<String>(&args, 0)?,
            ...
            "backup:deleteTempBackup" => to_value(commands::backup::delete_temp_backup(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "system:getGitBashPath" => {
                to_value(commands::system::system_get_git_bash_path(&state)?)
            }
            "system:getGitBashPathInfo" => {
                to_value(commands::system::system_get_git_bash_path_info(&state)?)
            }
            "system:setGitBashPath" => to_value(commands::system::system_set_git_bash_path(
                &state,
                opt_arg::<String>(&args, 0)?,
            )?),
            ...
            "miniwindow:set-pin" => to_value(commands::mini_window::mini_window_set_pin(
                &app,
                arg::<bool>(&args, 0)?,
            )?),
            ...
            // Trace
            "trace:saveData" => to_value(commands::trace::trace_save_data(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "trace:saveEntity" => to_value(commands::trace::trace_save_entity(
                &state,
                arg::<Value>(&args, 0)?,
            )?),
            "trace:getEntity" => to_value(commands::trace::trace_get_entity(
                &state,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "trace:setTitle" => to_value(commands::trace::trace_set_title(
                &window,
                arg::<String>(&args, 0)?,
            )?),
            ...
            "cherryai:get-signature" => {
                to_value(commands::cherryai::cherryai_get_signature(arg::<
                    commands::cherryai::CherryAiSignatureParams,
                >(
                    &args, 0
                )?)?)
            }
