...
fn ensure_key_32(secret_key: &str) -> Result<[u8; 32]> {
  ...
  if bytes.len() != 32 {
    return Err(DromeError::Message("secretKey must be 32 bytes for aes-256-cbc".into()));
  }
  let mut out = [0u8; 32];
...
fn parse_iv_hex(iv_hex: &str) -> Result<[u8; 16]> {
  let iv_bytes = hex::decode(iv_hex).map_err(|e| DromeError::Message(format!("Invalid iv hex: {e}")))?;
  if iv_bytes.len() != 16 {
    return Err(DromeError::Message("iv must be 16 bytes (32 hex chars)".into()));
  }
  let mut out = [0u8; 16];
...
pub fn aes_decrypt(encrypted_hex: String, iv_hex: String, secret_key: String) -> Result<String> {
  ...
  let mut buf = hex::decode(encrypted_hex).map_err(|e| DromeError::Message(format!("Invalid encrypted hex: {e}")))?;

  let plaintext = Decryptor::<Aes256>::new(&key.into(), &iv.into())
    .decrypt_padded_mut::<Pkcs7>(&mut buf)
    .map_err(|e| DromeError::Message(format!("Decrypt failed: {e}")))?;

  String::from_utf8(plaintext.to_vec()).map_err(|e| DromeError::Message(format!("Invalid utf8: {e}")))
}
