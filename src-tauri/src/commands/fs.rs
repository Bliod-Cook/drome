...
pub fn fs_read(app: &AppHandle, state: &State<'_, AppState>, path_or_url: String, encoding: Option<String>) -> Result<Value> {
  if path_or_url.starts_with("http://") || path_or_url.starts_with("https://") {
    return Err(DromeError::Message("fs.read for http(s) URLs is not implemented".into()));
  }
...
pub fn fs_read_text(app: &AppHandle, state: &State<'_, AppState>, path_or_url: String) -> Result<String> {
  if path_or_url.starts_with("http://") || path_or_url.starts_with("https://") {
    return Err(DromeError::Message("fs.readText for http(s) URLs is not implemented".into()));
  }
